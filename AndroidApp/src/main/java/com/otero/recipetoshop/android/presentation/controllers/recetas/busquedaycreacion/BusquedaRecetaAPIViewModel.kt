...
import com.otero.recipetoshop.Interactors.recetas.cestacompra.AddRecetaCestaCompra
...
constructor(
    ...
    fun onTriggerEvent(event: BusquedaRecetasAPI){
        when(event){
            is BusquedaRecetasAPI.onAddUserReceta -> {
                addRecetaUserToCestaCompra(event.nombre, event.cantidad)
            }
            is BusquedaRecetasAPI.buscarRecetas -> {
                busquedaRecetasAPIState.value = busquedaRecetasAPIState.value.copy(lisaRecetasBuscadas = emptyList())
                buscarRecetasAPI()
            }
            is BusquedaRecetasAPI.updateQuery -> {
                updateQueryRecetas(event.newQuery)
            }
            is BusquedaRecetasAPI.onAddYummlyReceta -> {
